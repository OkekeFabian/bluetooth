...
class BluetoothTerminal {
  ...
  _maxCharacteristicValueLength = 20;
  _serviceUuid = 0xFFE0;
  ...
  constructor(serviceUuid = 0xFFE0, characteristicUuid = 0xFFE1, receiveSeparator = '\n', sendSeparator = '\n', onConnected = null, onDisconnected = null) {
    ...
    this._boundHandleCharacteristicValueChanged = this._handleCharacteristicValueChanged.bind(this); // Configure with specified parameters.
  ...
  receive(data) {} // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  send(data) {
    ...
    data = String(data || ''); // Return rejected promise immediately if data is empty.

    if (!data) {
      return Promise.reject(new Error('Data must be not empty'));
    }

    data += this._sendSeparator; // Split data to chunks by max characteristic value length.

    const chunks = this.constructor._splitByLength(data, this._maxCharacteristicValueLength); // Return rejected promise immediately if there is no connected device.
    ...
    let promise = this._writeToCharacteristic(this._characteristic, chunks[0]); // Iterate over chunks if there are more than one of it.
  ...
  _handleDisconnection(event) {
    ...
    this._connectDeviceAndCacheCharacteristic(device).then(characteristic => this._startNotifications(characteristic)).then(() => {
      if (this._onConnected) {
        this._onConnected();
      }
...
exports.default = BluetoothTerminal;